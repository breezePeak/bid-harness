...
describe('dsh-base bundle', () => {
  it('declares a parseable patch list through the dsh.bundle.patch manifest field', () => {
    ...
    expect(rows.filter(row => row.id === 'web-search-deepseek')).toHaveLength(1)
    expect(rows.filter(row => row.id === 'web-search-tavily')).toHaveLength(1)
    expect(rows.find(row => row.id === 'web')?.config?.['searchProvider']).toBe('deepseek-official')
    expect(rows.find(row => row.id === 'web')?.config?.['fetchProvider']).toBe('http')
    expect(manifest.dependencies).not.toHaveProperty('@deepseek-ai/dsh-subagent-codex')
    expect(manifest.dependencies).not.toHaveProperty('@deepseek-ai/dsh-subagent-claude-code')
    expect(manifest.dependencies).toHaveProperty('@deepseek-ai/dsh-web-fetch-http', 'workspace:^')
    expect(manifest.dependencies).toHaveProperty('@deepseek-ai/dsh-web-search-deepseek', 'workspace:^')
    expect(manifest.dependencies).toHaveProperty('@deepseek-ai/dsh-web-search-tavily', 'workspace:^')
  })
