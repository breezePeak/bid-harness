...
Web Service、DeepSeek 与 Tavily Search Provider，以及匿名 HTTP Fetch Provider 挂载在 Host Plane；模型工具由基础组合或 Agent Preset 决定是否暴露。基础 Web Service 选择 `deepseek-official` 搜索和 `http` 抓取 Provider。用户可以在 profile 中将 `searchProvider` 改为 `tavily`；所选 Provider 缺失或重复时，组合加载或工具调用会明确失败，Tavily 凭据缺失时搜索调用会报告凭据问题。
...
- **HTTP Fetch Provider 尚未阻止私网、回环、链路本地或重绑定地址**——启用 `web_fetch` 的部署必须把运行网络视为可被模型选择 URL 访问；不得让 Harness 进程直接访问敏感内部网络。完整限制见 [`@deepseek-ai/dsh-web-fetch-http`](../../web/web-fetch-http/README.zh.md#已知限制与暂缓事项)。
