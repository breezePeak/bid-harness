...
AgentLoop 在记录和组装每个 `tool-call-delta` 前按 call id 检查未闭合 arguments。连续 128 个完全相同的非空片段以 `MODEL_TOOL_ARGUMENT_DEGENERATED` 失败，累计超过 512 KiB 以 `MODEL_TOOL_ARGUMENT_TOO_LARGE` 失败；关闭迭代器会等待提供方 stream 清理，这两个代码不在默认重试集合中。
