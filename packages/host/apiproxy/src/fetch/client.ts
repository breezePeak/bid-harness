...
import { webDiagnoseValueSchema } from '../api/web.schema.ts'
import {
...
export interface IApiClient {
  ...
  web: {
    diagnose(payload: RequestPayload<'web.diagnose'>, signal?: AbortSignal): Promise<RpcResponse<ResponseValue<'web.diagnose'>>>
  }
...
const UNARY_VALUE_SCHEMAS: { [K in keyof RpcMethodMap]: z.ZodType<Wire<ResponseValue<K>>> } = {
  ...
  'web.diagnose': webDiagnoseValueSchema,
}
...
export abstract class AbstractApiClient implements IApiClient {
  ...
  readonly web: IApiClient['web'] = {
    diagnose: (payload, signal) => this.callUnary('web.diagnose', payload, signal),
  }
