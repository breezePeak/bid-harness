...
describe('runtime client apply', () => {
  it('mounts slots, Sessions, and Workspaces and fans host frames into both managers', async () => {
    ...
    expect(sessions).toBeInstanceOf(SessionRuntime)
    expect(workspaces !== undefined).toBe(true)
    // The bound the wire schema enforces, not a per-connection negotiation.
    if (!(sessions instanceof SessionRuntime)) throw new Error('SessionRuntime missing after runtime apply')
    expect(sessions.searchResultLimit).toBe(SESSION_SEARCH_RESULT_LIMIT)
    ...
    expect(sessions.list.getSnapshot().ids).toContain('s-new')
    bench.sinks?.onHostEnvelope?.({
  ...
  it('selects the recent Workspace once when the first baselines have no current session', async () => {
    ...
    const workspaces = bench.ctx.get('workspaces')
    if (!(sessions instanceof SessionRuntime)) throw new Error('SessionRuntime missing after runtime apply')
    if (!(workspaces instanceof WorkspaceRuntime)) throw new Error('WorkspaceRuntime missing after runtime apply')
