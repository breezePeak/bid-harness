...
On a started session, choosing a preset creates and opens a fresh blank session in the same Workspace, then applies the staged preset there. The original session retains its composition. The chip continues to name the current session's committed preset while creation or selection is pending; a Host failure restores that value and reports the failure.
...
Beside copying sits the conversational entry: when the roster carries the self-referential `cordis` preset, a dashed add-card (the Models page's affordance) stages it and starts a new session — the section closes the settings panel through the shell's owner-prop `close` and the new-session chip's own applier composes the blank session the workspace flow produces. The seat keeps a late roster load on the current session's committed composition; only the no-session screen shows a staged pick.
