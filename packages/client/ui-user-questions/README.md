...
Web Question 功能的浏览器端把 `question` entry 注册到 conversation-owned 的 `conversation.composer.interaction` chain。Question 面板位于 ordinary chat InputBar 上方，两者同时可见；回答仍通过原有 `PendingQuestion` carrier 和批量 answer wire shape 传输。Host 端有意保持为空：渲染 Question 是 Host UI 能力，拥有 `dsh-tool-ask-user` 则是 Agent 能力，因此工具仍由需要它的 preset 和没有 preset 的 TUI composition 组装。
...
选择状态保存在以 request rpcId 为 key 的组件内；相同 id 的 replay 保留仍挂载的草稿，Host 的 `question/resolved` 移除 Question 面板。Host 保持权威：HTTP 投递成功不会在本地提前删除 pending 状态，也不会改变 ordinary chat InputBar。
