...
本包自带的卡片覆盖 shell 执行器（`bash`）、agent 循环的工具调用并行度（`agent-loop`）以及 Web 搜索（`web`）。Web 搜索默认跟随当前任务的模型 Provider；用户可以在卡片上方切换到独立搜索 Provider，并在下方编辑该独立搜索的连接与检索参数。API Key 通过 credentials 写入，不进入 settings 文件；`web-search-deepseek` 预算和 `web-search-tavily` 独立 Provider 配置由同一张卡片暂存、保存和放弃。
