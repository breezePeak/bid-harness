...
function composer() {
  ...
  const forward = vi.fn(async () => ({ kind: 'success' as const }))
  ...
  const props = {
    ...
    useProjection: () => ({ task: { stage: 'chapter_writing', status: 'completed', run: null } }),
    ...
    actions: store.actions, getChapter, registerSubmit,
...
it('章节拖入生成独立标签，引用随普通消息交给主 Agent 判断，成功后释放引用', async () => {
  ...
  expect(view.forward).toHaveBeenCalledWith(
