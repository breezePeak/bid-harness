...
it('章节拖入生成独立标签，引用随普通消息交给主 Agent 判断，成功后释放引用', async () => {
  ...
  expect(view.sendMessage).toHaveBeenCalledWith(expect.stringContaining('"kind":"bid_chapter_reference"'), 'queue', undefined)
  expect(view.sendMessage).toHaveBeenCalledWith(expect.stringContaining('请最小修改'), 'queue', undefined)
