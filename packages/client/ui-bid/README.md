...
章节写作工作台支持点击各级父节点，在阅读区标题下查看 S4 已确认的简短概述，了解下属章节的主要内容；父节点状态标为“章节概述”，资料栏引导查看子章节依据。默认优先打开已有正文的叶节，尚无正文时可阅读父节点概述；刷新保留当前章节。父节点概述不提供正文修订引用。正文详情中的普通正文段落按 2 字符首行缩进显示。写作工作台按祖先到当前章节的 `order` 显示完整层级编号，与正文标题编号一致；折叠不改变编号，悬停显示完整编号与标题。
