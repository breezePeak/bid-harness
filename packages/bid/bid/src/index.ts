...
export { BID_CLIENT_ACTIONS, BID_DOCUMENT_ROLES, BID_RUNTIME_PROJECTION_KEY, BID_STAGES, BID_WORK_KINDS, BID_WORKFLOW_GATES, STAGE_RUN_STATUSES, isBidDocumentRole, parseBidReviewWorkbenchView } from './control-plane-contract.ts'
...
function bidSessionRuntime(session: Session): BidRuntimeState {
  return bidRuntimeView(bidSessionControlState(session))
}
...
async function ensureWritingRequirementsRequested(
  ...
  if (raw !== undefined) {
    ...
    if (existing.confirmed_outline_sha256 === sha256
      && (existing.state === 'awaiting_answer' || existing.state === 'answered')) {
      return existing
    }
    if (existing.confirmed_outline_sha256 === sha256 && existing.state === 'dismissed' && !reopenDismissed) return existing
  }
...
async function executeOutlineRegenerationCandidate(
  ...
  try {
    ...
    return { ok: true, value: { stage: run.work.stage, status: 'waiting_user' } }
  } catch (error: unknown) {
    return { ok: false, error: {
...
export class BidHostRuntime extends TypertRemoteService {
  ...
  private async prepareOperation(operation: ActiveBidOperation): Promise<BidRuntimeState> {
    ...
    let state = saved
    if (state === undefined) state = await checkpointBidProjectState(operation.workspace, BID_INITIAL_CONTROL_STATE)
    else if (state.run?.status === 'running' || state.run?.status === 'cancelling') {
      const interrupted = {
        ...state.run,
        status: 'suspended' as const,
        cause: 'host_restart' as const,
        updatedAt: Date.now(),
      }
      state = await checkpointBidProjectState(operation.workspace, {
    ...
    operation.session.append('bid.project.resumed', {
      workflow: state.workflow, run: state.run, lastRun: state.last_run, revision: state.revision,
    })
  ...
  private startWritingQuestion(agent: Agent, request: WritingRequest): void {
    ...
    pending.task = this.ctx.userQuestions.ask({
      ...
      if (code === 'ASK_ABORTED' || controller.signal.aborted) {
  ...
  private async acceptWritingQuestionAnswer(
    ...
    try {
      ...
      if (request?.request_id !== pending.requestId || request.attempt_id !== pending.attemptId
        ...
        || runtime.stage !== 'chapter_writing' || runtime.status !== 'waiting_user'
      ...
      const next: WritingRequest = classified.kind === 'dismissed'
        ? { ...request, state: 'dismissed', error: undefined }
        : {
          ...request,
          state: 'answered',
          error: undefined,
    ...
    if (accepted && request !== undefined) this.scheduleWritingPlanProcessing(pending.agent, request)
  }
  ...
  private async mutateProject(
    ...
    control = operation.session.events.reduce(reduceBidControlState, BID_INITIAL_CONTROL_STATE),
  ): Promise<void> {
    const state = await commitBidProjectMutation(operation.workspace, operation.projectRevision, control, mutate)
    operation.projectRevision = state.revision
    await this.publishProjectState(operation, state)
  }
  ...
  constructor(ctx: Context, config: Config = DEFAULT_HOST_RUNTIME_CONFIG) {
    ...
    ctx.on('agent/cancel-requested', ({ agent, cause }) => {
      ...
      this.invalidateWritingQuestion(projectKey(agent.session))
      const operation = this.inFlight.get(projectKey(agent.session))
  ...
  > {
    ...
    await Promise.all([markerPath, appliedPath].map(path => assertNoLinkedPath(workspace.root, path)))
    const marker = submitted.update_kind === 'initial'
      ? writingRequestSchema.parse(JSON.parse(await readFile(markerPath, 'utf8')))
      : await readWritingRequest(workspace)
    const current = await confirmedOutline(workspace)
    if (marker !== undefined && marker.confirmed_outline_sha256 !== current.sha256) throw new Error('写作要求询问与当前确认目录不一致，请重新进入 S5。')
    const previous = await currentWritingPlan(workspace)
    const input = submitted
    if (submitted.update_kind === 'initial') {
      if (marker === undefined || submitted.writing_request_id !== marker.request_id) {
        return { ok: false, error: { code: 'BID_WRITING_PLAN_INVALID', issues: ['writing_request_id 已失效，请重新读取 S5 任务上下文。'] } }
      }
  ...
  private async executeStageInteraction(agent: Agent, input: unknown, callerSignal: AbortSignal): Promise<unknown> {
    ...
    if (request.action === 'bid_confirm_writing_plan') {
      ...
      const saved = await readBidProjectState(workspace)
      if (saved?.run?.status === 'suspended' && saved.run.stage === 'chapter_writing'
        && saved.run.work.kind === 'stage_execution') {
        const operation = this.beginOperation(session)
        try {
          await this.prepareOperation(operation)
          const current = operation.session.events.reduce(reduceBidControlState, BID_INITIAL_CONTROL_STATE).run
          if (current?.status !== 'suspended' || current.runId !== saved.run.runId) {
    ...
    try {
      ...
      const writingPlanAction = request.action === 'bid_confirm_writing_plan' && runtime.stage === 'chapter_writing'
        && (runtime.status === 'waiting_user'
          || (request.update_kind === 'patch' && (runtime.status === 'completed' || runtime.status === 'attention_required')))
  ...
  private async driveStartedSession(agent: Agent, cwd: string): Promise<void> {
    ...
    try {
      ...
      if (runtime.stage === 'chapter_writing' && runtime.status === 'waiting_user') {
        const request = await ensureWritingRequirementsRequested(String(session.id), workspace, request => this.mutateProject(
          operation,
          lease => writeWritingRequest(workspace, request, lease),
        ))
        await this.ctx.sessions.flush(session)
        if (request !== undefined) void operation.done.then(async () => {
          if (request.state === 'awaiting_answer') this.startWritingQuestion(agent, request)
          else await this.resumeWritingPlanProcessing(agent, workspace)
      ...
      if (next.stage === 'chapter_writing' && next.status === 'waiting_user') {
        const request = await ensureWritingRequirementsRequested(String(session.id), workspace, request => this.mutateProject(
          operation,
          lease => writeWritingRequest(workspace, request, lease),
        ))
  ...
  async autoStartChapterWriting(session: Session): Promise<BidChapterWritingGateResult> {
    ...
    try {
      ...
      if (writingRequest?.state === 'answered') {
        return chapterWritingGateResult({ ok: false, code: 'BID_CHAPTER_WRITING_GATE_FAILED', message: '已有自定义写作要求待处理，请先提交其 Writing Plan。' })
      }
      if (writingRequest?.state === 'awaiting_answer') {
        this.invalidateWritingQuestion(projectKey(session))
        const dismissed = { ...writingRequest, state: 'dismissed' as const, error: undefined }
        await this.mutateProject(operation, lease => writeWritingRequest(operation.workspace, dismissed, lease))
      }
  ...
  async resumeCurrentRun(
    ...
    try {
      const runtime = await this.prepareOperation(operation)
      const suspended = operation.session.events.reduce(reduceBidControlState, BID_INITIAL_CONTROL_STATE).run
      if (operation.projectRevision !== expectedProjectRevision || suspended?.status !== 'suspended'
  ...
  private async resumeDedicatedWork(
    ...
    try {
      ...
      if (run.work.kind === 'evidence_remap' || run.work.kind === 'outline_regeneration') {
        operation.session.append('bid.user_confirmation.required', {
          stage: run.work.stage,
          status: 'waiting_user',
        })
      }
  ...
  async exportDocx(session: Session, templateId: DocxTemplateId | null): Promise<BidDocxExportResult> {
    ...
    try {
      return await this.withDocxOperation(session, async (workspace) => {
        ...
        const control = active === undefined
          ? saved === undefined
            ? session.events.reduce(reduceBidControlState, BID_INITIAL_CONTROL_STATE)
            : { workflow: saved.workflow, run: saved.run, lastRun: saved.last_run }
  ...
  async getOutlineDraft(session: Session): Promise<OutlineDraftView> {
    ...
    const runtime = state?.runtime ?? BID_INITIAL_RUNTIME_STATE
  ...
  async confirmOutline(session: Session, request: OutlineDraftIdentityRequest): Promise<BidOutlineConfirmationResult> {
    ...
    try {
      ...
      enteredWritingGate = next.stage === 'chapter_writing' && next.status === 'waiting_user'
      await this.ctx.sessions.flush(session)
      ...
      await this.finishOperation(session, operation)
      if (enteredWritingGate) void operation.done.then(() => this.requestWritingRequirements(session)).catch((error: unknown) => {
        this.ctx.logger.warn(`Bid S5 原生提问启动失败：${String(error)}`)
      })
    }
  }
  ...
  async regenerateOutline(
    ...
    try {
      ...
      const work = await persistHostWork(workspace, 'outline_regeneration', runtime.stage, workRequest)
      const admittedRun = await operation.runs.start(work)
      run = admittedRun
