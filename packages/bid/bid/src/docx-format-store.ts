...
const requestSchema = z.strictObject({ revision: z.number().int().nonnegative(),
  ...
  template: z.strictObject({ name: z.string().max(200).regex(/\.docx$/iu),
    data: z.string().max(14 * 1024 * 1024) }).optional() })
...
const stateSchema = requestSchema.omit({ template: true }).extend({ version: z.literal(1),
  opened: z.boolean(),
...
export async function readDocxFormat(workspace: BidWorkspace): Promise<DocxFormatView> {
  ...
  return withTenderWarnings(workspace, resolveFormat(parsed.data, formatFields(workspace.config)))
}
...
export async function saveDocxFormat(workspace: BidWorkspace, request: DocxFormatRequest): Promise<DocxFormatView> {
  ...
  if (!parsed.success)
    throw new Error('Word 配置请求无效，请检查文件类型、大小和字段。')
  ...
  if (input.template) {
    const bytes = Buffer.from(input.template.data, 'base64')
    if (bytes.toString('base64') !== input.template.data)
      throw new Error('模板文件编码无效。')
    ...
    if (current.state.template?.hash !== hash) {
      ...
      state.template = cached === undefined ? await parseDocxTemplate(bytes, input.template.name) : templateSchema.parse(JSON.parse(cached))
  ...
  const result = resolveFormat(state, current.fields)
