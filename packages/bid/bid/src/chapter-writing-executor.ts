...
export function renderChapterSubagentTask(
  ...
  return [
    ...
    '段落第一句话只有在概括本段结论、总体措施或核心安排时才使用 **总结句。** 的 Markdown 加粗；只加粗这句，后续展开句保持普通正文。第一句话不是总结句时不要加粗。不得把整段、多句、列表、表格或全文统一加粗，也不得在段落中后部随意加粗。标题及普通正文的字体、字号和字重仍由 Word 模板控制。',
...
function renderChapterReviewerTask(
  ...
  return [
    ...
    '检查正文加粗是否遵守以下边界：只有段落第一句话确实概括本段结论、总体措施或核心安排时，才允许只加粗该总结句；普通开场句不加粗，后续展开句、整段、多句、列表、表格和全文不得统一加粗。发现滥用时将 structure_complete 设为 false，并在 blocking_issues 指明具体段落；合法的段首总结句加粗不构成问题。',
