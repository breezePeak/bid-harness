...
export function renderOutlineGenerationRepairTask(
  ...
  return [
    ...
    '修正目录树、叶子粒度、ID 覆盖和质量报告后，将 reviewed_section_ids 更新为最终全部 section ID。可写叶子的 requirement_ids 最多 4 个、scoring_ids 最多 3 个；索引或附录不能替代实质性正文叶子的映射。仅在所有问题已解决时写 issues=[]。',
