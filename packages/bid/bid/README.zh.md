...
Tender Analysis 与 Evidence Mapping Executor 都通过 `Agent.followup()` 注入包含 Session 相对路径和当前 schema 的动态任务，并按 Stage Policy 限制本轮工具。S2 只将技术标范围的 Requirement、Scoring 和 Compliance 写入正式 Artifact，纯商务、资格和报价内容不会进入 S3。S3 主 Agent 只读取 manifest 与 S2 Artifact，完成全局分析并按业务主题动态写入内部 Mapping Task 计划；Host 校验 ID 覆盖后，在并发上限内为每个任务启动独立的 fresh-context Subagent。每个 Child 仅获得当前任务所需的 S2 切片和 Corpus 定位，工具限制为 `grep`、`read`、`web_search`、`web_fetch`，本地检索必须先定位再读取，联网检索必须先搜索再抓取，并通过 `outputSchema` 返回局部结构化结论。Host 独立校验每个结果，失败时只以新的 Child 修复该任务，已通过的兄弟任务不会重跑；最终按稳定 ID 合并去重后交由主 Agent 生成正式 `analysis/evidence-map.json`。
