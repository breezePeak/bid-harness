...
Tender Analysis 与 Evidence Mapping Executor 都通过 `Agent.followup()` 注入包含 Session 相对路径和当前 schema 的动态任务，并按 Stage Policy 限制本轮工具。S2 只将技术标范围的 Requirement、Scoring 和 Compliance 写入正式 Artifact，纯商务、资格和报价内容不会进入 S3。S3 主 Agent 只读取 manifest 与 S2 Artifact，完成全局分析并按业务主题动态写入内部 Mapping Task 计划；计划文件必须是唯一完整的严格 JSON。Host 将计划缺失、JSON 语法错误和字段 Schema 错误分别作为 `EVIDENCE_MAPPING_PLAN_MISSING`、`EVIDENCE_MAPPING_PLAN_JSON_INVALID`、`EVIDENCE_MAPPING_PLAN_SCHEMA_INVALID` 记录；Schema 问题保留字段路径，修复前删除旧计划文件。Host 校验 ID 覆盖后，在并发上限内为每个任务启动独立的 fresh-context Subagent。Host 从内部执行日志计算总数、已完成、映射中、未开始和失败数，并由 `bid/getEvidenceMappingProgress` 在 S3 运行期间提供给主 Agent 页面。每个 Child 仅获得当前任务所需的 S2 切片和 Corpus 定位，工具限制为 `grep`、`read`、`web_search`、`web_fetch`，本地检索必须先定位再读取，联网检索必须先搜索再抓取；返回前在当前任务内核对任务 ID、三类分配 ID 的唯一完整覆盖、已读取的本地引用和当前任务的 Search-to-Fetch 外部引用，再通过 `outputSchema` 返回局部结构化结论。Host 独立校验每个结果，失败时只以新的 Child 修复该任务，已通过的兄弟任务不会重跑；最终按稳定 ID 合并去重后交由主 Agent 生成正式 `analysis/evidence-map.json`。
