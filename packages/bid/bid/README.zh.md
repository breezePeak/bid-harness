...
Host 插件注册该 Projection，并全局拒绝已解析 Preset 为 `bid` 的 Session 进入通用 Prompt 路径。`evidenceMappingMaxConcurrency` 和 `chapterWritingMaxConcurrency` 分别限制 S3 Mapping Subagent 与 S6 Chapter Subagent 的同时运行数量，均默认为 3，可配置为 1–8。

`bid` Agent Preset 只为 Bid Session 注册 `/bid-reset-s2` 至 `/bid-reset-s7` 六个无参数命令。每个命令只能重置同名的当前阶段，追加 `bid.stage.reset` 后从阶段入口完整重跑；正在被 Host 操作拥有的 Session 会拒绝重置。S5 重置还会删除可编辑草稿和部分发布的确认文件，然后从 S4 目录重建初始草稿。命令结果不进入模型历史。
...
Tender Analysis 与 Evidence Mapping Executor 都通过 `Agent.followup()` 注入包含 Session 相对路径和当前 schema 的动态任务，并按 Stage Policy 限制本轮工具。S2 只将技术标范围的 Requirement、Scoring 和 Compliance 写入正式 Artifact，纯商务、资格和报价内容不会进入 S3。S3 主 Agent 只读取 manifest 与 S2 Artifact，完成全局分析并按业务主题动态写入内部 Mapping Task 计划；计划文件必须是唯一完整的严格 JSON。Host 将计划缺失、JSON 语法错误和字段 Schema 错误分别作为 `EVIDENCE_MAPPING_PLAN_MISSING`、`EVIDENCE_MAPPING_PLAN_JSON_INVALID`、`EVIDENCE_MAPPING_PLAN_SCHEMA_INVALID` 记录；Schema 问题保留字段路径，修复前删除旧计划文件。Host 校验 ID 覆盖后，在并发上限内为每个任务启动一个 fresh-context、可继续的 Child。Host 从内部执行日志计算总数、已完成、映射中、未开始和失败数，并由 `bid/getEvidenceMappingProgress` 在 S3 运行期间提供给主 Agent 页面。每个 Child 仅获得当前任务所需的 S2 切片和 Corpus 定位，工具限制为 `grep`、`read`、`web_search`、`web_fetch`，本地检索必须先定位再读取，联网检索必须先搜索再抓取；最终轮只能返回原始 JSON，Host 以严格 Schema、已读取的本地引用、当前任务的 Search-to-Fetch 外部引用和三类分配 ID 的唯一完整覆盖校验。校验失败时，Host 将具体字段问题交给同一 Child 的后续轮次，已通过的兄弟任务不会重跑；Host 对能唯一匹配 manifest 前缀的截断或重复 Corpus `file_id` 还原为完整 id，再校验 chunk，避免模型手抄不透明哈希导致无意义的失败。最终按稳定 ID 合并去重后交由主 Agent 生成正式 `analysis/evidence-map.json`。
...
S3 Child 只在公开技术知识缺口存在时执行 `web_search → web_fetch`；`external_materials` 只保存该 Child 搜索结果中出现、随后成功抓取 2xx 正文且与 Host 来源账本匹配的公开技术来源，不能替代企业事实的本地证据。Host 不向 Child 传递 S2 的招标来源引用；招标文件只用于 S2 产物中的需求解释，Child 既不能读取其分块，也不能将其作为本地 Evidence。Host 汇总所有 Child 的规范 Tool 结果与 Session Tool Event，写入 `analysis/web-evidence-sources.json`，并把实际交给 Child 的有界抓取文本保存到 `analysis/web-sources/<source_id>.md`，不发起第二次 HTTP 请求。Validator 按 `file_id + chunk id` 识别本地证据，由分块索引解析规范路径和校验行号；提交值中的相对路径、绝对路径或分隔符差异不参与身份判断。人工框架标题允许没有 mapping，同一 `file_id + source_section_id` 最多一个 mapping。没有联网时 Host 生成空来源账本，本地资料或 `missing_topics` 仍可正常通过；工具或 Provider 未注册会使阶段明确失败，搜索、抓取、超时、取消、非 2xx 或空正文都不会形成来源。通用重试会清除旧计划、执行日志、evidence map、账本和快照。
...
S6 Stage Policy 声明 `grep`、`read`、`write`、`web_search` 和 `web_fetch` 的阶段能力合集，Executor 再按角色收窄：主 Agent 只能读取输入并写 `chapters/execution-plan.json`，Writer Child 只能使用 `grep`、`read`、`web_search` 和 `web_fetch`，Reviewer Child 没有工作区或网络工具。Host 校验计划完整覆盖、引用合法且无环后，按确认目录的稳定顺序维护 ready queue，并通过 `ctx.subagents.start('spawn', …)` 在并发上限内启动独立 Child Session；强依赖章节只在前置候选通过后启动，并且只接收有界 handoff 和依赖原因。Writer 先消费章节相关 S3 Evidence，资料不足时自主补搜本地资料，仅对仍缺少的公开技术知识执行 `web_search → web_fetch`，不要求每章联网。Host 把成功的 Search-to-Fetch 调用、章节 ID、Writer Child Session、抓取时间、URL、正文哈希和正文快照追加到 `analysis/web-evidence-sources.json` 与 `analysis/web-sources/`，保留 S3 记录；`additional_external_materials` 只接受与该持久记录匹配的来源。Child 通过 `outputSchema` 返回 Markdown 与 Metadata 候选，Host 校验后原子写入正文与 sidecar；失败修复使用新的 Writer Child，不回退到主 Agent。Host 写入 `chapters/execution-log.json` 记录每次 Child Session、停止原因和最终接受者，再生成 `chapters/manifest.json`。重试 S6 会先删除旧 `chapters/` 树并重新规划。企业案例、资质、人员、实有产品参数和既有能力缺失时必须保留 `unresolved_topics`，不得用 Web 资料替代。
