...
S2 writes only technical-bid Requirements, Scoring, Compliance, and the Host-owned stable response-point Catalog; commercial, qualification, and price-only material never reaches S3. The Catalog binds ordered `RP-*` identities to the canonical scoring hash. S3 Evidence Map schema v6 identifies local material by exact `file_id + chunk_XXXX`; the Host resolves ownership, canonical paths, linked-path safety, and framework or reference-bid section metadata. Mapping Children produce semantic relationships, while the Host deduplicates their results and writes the final Artifact without a whole-stage model merge or repair. S4 and S5 carry stable mapping and response-point IDs; S6 resolves source headings from the original structure Artifact before reading complete Chunk text.
...
In S6, the Main Agent writes only the relation plan. The Host validates it, resolves each assigned framework or reference-bid Mapping into bounded Chunk excerpts, schedules fresh-context Writer Children according to its strong-dependency DAG, and then starts an independent Reviewer Child for every candidate. The Writer receives only the current section inputs, response-point mappings, resolved source excerpts, and bounded prerequisite handoffs. The Reviewer receives only Host-injected inputs and structured output. The Host checks reviewer quotes against the candidate before atomically persisting the Markdown, Metadata, review report, and manifest entry. New Writer Children handle repairs; the Main Agent never writes fallback chapter content. Missing enterprise cases, qualifications, personnel, actual product parameters, or existing capabilities must remain in `unresolved_topics` and cannot be replaced with Web sources.
...
- S6 receives cancellation only from its owned execution failure path; wiring parent-session stop signals into the chapter controller remains pending.
- S7 verifies stable structured response-point declarations but does not perform a second whole-book semantic review.
