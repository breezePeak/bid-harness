...
describe('outline-confirmation rule ownership', () => {
  it('registers complete S5 issue metadata', () => {
    ...
    expect(Object.keys(OUTLINE_CONFIRMATION_ISSUES).some(code => code.includes('SOURCE_MAPPING'))).toBe(false)
  })
  ...
  it('does not import or call the S4 generation-quality validator', async () => {
    ...
    expect(source).not.toContain('evidence-map.json')
    expect(source).not.toContain('parseEvidenceMapArtifact')
  })
})
