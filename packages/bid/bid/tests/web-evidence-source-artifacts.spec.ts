...
describe('Web evidence source Artifact schema', () => {
  ...
  it('derives deterministic content hashes and filename-safe source ids', () => {
    ...
    expect(webEvidenceSourceId('https://example.com/', hash)).toMatch(/^WEB-[a-f0-9]{16}$/u)
    expect(webEvidenceSourceId('https://example.com/', hash)).toBe(webEvidenceSourceId('https://example.com/', hash))
  })
})
