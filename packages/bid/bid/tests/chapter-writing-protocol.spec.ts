...
describe('S5 Reviewer 分批记录', () => {
  ...
  it('preferred 语义与确定性条件未满足时保留结果但不阻断章节通过', async () => {
    ...
    context.sectionWritingPlan = {
      ...context.sectionWritingPlan,
      acceptance_criteria: [{
        id: 'AC-PREFERRED-SEMANTIC', scope: { kind: 'section', section_id: context.section.id },
        description: '尽量补充更多实施示例。', priority: 'preferred', evaluator: { kind: 'semantic' },
      }, {
        id: 'AC-PREFERRED-METRIC', scope: { kind: 'section', section_id: context.section.id },
        description: '篇幅尽量更充分。', priority: 'preferred',
        evaluator: { kind: 'deterministic', metric: 'character_count', min: 1000, max: null },
      }],
    }
  ...
  it('全局 Compliance 与章节 Checklist 分离，重复文本仍按独立身份组装', async () => {
    const { ctx, agent, call } = await harness()
    ...
    expect(report.compliance_coverage[0]?.item).toBe('本章必须说明审计')
    expect(report.global_compliance_checks).toEqual([{ compliance_id: 'GLOBAL-1', item: '全局安全约束', status: 'not_applicable', evidence_quotes: [], issue: '本章不涉及该约束。' }])
  ...
  it('全局约束只有真实违规才形成 repair，外部资料缺口单独 attention 且不要求扩写', async () => {
    const { agent, call } = await harness()
    const context = reviewContext()
    ...
    const attention = attachChapterReview(agent, context, new Map([['Q1', '正文违反全局规则']]), evidence, 0)
    await call('review_coverage_items', { items: buildChapterReviewChecklist(context).map(item => covered(item.item_ref)) })
    ...
    expect(attention.captured()).toMatchObject({
      verdict: 'attention', blocking_issues: [],
      external_input_gaps: [{ item_ref: 'R1', required_material: '企业资质证书' }],
    })
    const report = attention.captured()!
    const invalid = { ...report, external_input_gaps: [{ ...report.external_input_gaps[0]!, item_ref: 'R2' }] }
    expect(validateChapterReview(context, {
      ...
      metadata: parseChapterMetadata({
        ...
        covered_scoring_response_point_ids: [], covered_scoring_response_points: [],
        local_materials_used: [], web_materials_used: [], unresolved_topics: [],
        handoff: { section_id: context.section.id, decisions: [], terminology: [], numbers_and_parameters: [],
...
describe('S5 整书动态验收', () => {
  it('Final Main Agent 记录 document acceptance 风险但不能提交 section 结论', async () => {
    ...
    const attention = attachChapterWritingCompletionReview(agent, plan, [{
      section_id: 'SEC-1',
      review: { verdict: 'attention', blocking_issues: [], acceptance_criteria_results: [] },
    }], bodies, [], 0)
    expect((await call('submit_chapter_writing_completion_review', {
    ...
    const settledRepair = attachChapterWritingCompletionReview(agent, plan, [{
      section_id: 'SEC-1',
      review: { verdict: 'repair', blocking_issues: ['正文缺少设备数量依据。'], acceptance_criteria_results: [] },
    }], bodies, [], 0)
    expect((await call('submit_chapter_writing_completion_review', {
...
describe('S5 文档级全局合规核验', () => {
  ...
  it('保存内容缺失与递交待确认，绑定实际检查章节并允许多章共同负责', async () => {
    ...
    const compliance = { schema_version: 1 as const, compliance_items: [
      { id: 'GLOBAL-CONTENT', type: '材料', raw_text: '整份文档应包含证明材料', normalized_rule: '整份文档包含证明材料', severity: 'mandatory' as const, source_refs: [] },
      { id: 'GLOBAL-UPLOAD', type: '递交', raw_text: '截止前上传', normalized_rule: '截止前完成上传', severity: 'fatal' as const, source_refs: [] },
    ] }
    const chapters = outline.sections.filter(section => section.writable).map((section, index) => ({
      section_id: section.id, title: section.title, markdown: `正文 ${index + 1}`, candidate_sha256: String(index + 1).repeat(64),
    }))
    ...
    expect((await call('finish_global_compliance_review', {})).value).toEqual({ completed: true })
    const report = runtime.captured()!
    expect(report.items.map(item => item.status)).toEqual(['fail', 'pending'])
    expect(validateGlobalComplianceReview(report, outline, compliance, chapters, { files: [] } as never)).toEqual([])
    expect(validateGlobalComplianceReview(report, outline, compliance, [{ ...chapters[0]!, candidate_sha256: 'f'.repeat(64) }, ...chapters.slice(1)], { files: [] } as never))
      .toEqual(expect.arrayContaining([expect.objectContaining({ code: 'GLOBAL_COMPLIANCE_CHAPTER_STALE' })]))
  })
})
