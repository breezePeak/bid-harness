...
- Parent directories are created. A rename that reports `EPERM`, `EACCES`, or `EBUSY` is retried after 20, 50, 100, 200, 400, and 800 milliseconds without deleting the target. On final failure the temp is removed and the original transient error is rethrown; readers observe either the old or the new complete content.
