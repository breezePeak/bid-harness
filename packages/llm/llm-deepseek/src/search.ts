 ...
 * The LLM registry exposes this provider-private transport as hosted search.
 * @module @deepseek-ai/dsh-llm-deepseek/search
 */
 ...
 * (`https://api.deepseek.com`). The plugin derives this path from the model
 * endpoint unless a Provider search endpoint override is configured.
 */
...
export class DeepSeekSearchProvider {
  /** Model route owning this search transport. */
  ...
  /**
   * Check locally available connection facts without resolving a credential.
   * @returns whether local configuration can initiate a credential preflight.
   */
  available(): boolean {
  ...
  /**
   * Run an auxiliary native search and normalize its discovery results.
   * @param request - query and requested source count.
   * @param signal - cancellation covering credentials, transport, and response parsing.
   * @returns structured sources; generated answer text is excluded.
   */
  async search(request: WebSearchRequest, signal?: AbortSignal): Promise<WebSearchResult> {
    ...
    if (!response.ok) {
      ...
      try {
        ...
        if (detail !== undefined && detail.length > 0) message = `DeepSeek Provider search request failed: ${detail.replaceAll(apiKey, '[REDACTED]')}`
      } catch (error: unknown) {
    ...
    try {
      ...
      throw new WebError(`DeepSeek Provider returned an unprocessable response body: ${String(error).replaceAll(apiKey, '[REDACTED]')}`, 'WEB_PROVIDER_ERROR')
    }
  }
  ...
  private async apiKey(options: DeepSeekSearchProviderOptions, signal?: AbortSignal): Promise<string> {
    ...
    throw new WebError(
      ...
      + ' "apiKey" in the llm-deepseek.search config',
