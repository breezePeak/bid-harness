...
- `ctx.workspaceRegistry.delete(id)`：先停止并删除该 Workspace 账本中的全部 Session，再移除 Workspace 注册记录及对应的持久顺序条目。未知 id 返回 `false`，成功移除记录则返回 `true`。该操作绝不删除本地项目目录或用户文件；表写入失败时会恢复原顺序和此前发布的实体。
...
创建和删除操作会在记录和顺序可能分叉之前，先持久化明确的待处理变更标记。启动时只补全该标记所指明的变更，随后清除标记；没有标记的顺序／表不一致仍属于来源不明的损坏，并会明确报错。删除后重新注册同一路径会生成新的 Workspace id，不会恢复已删除的会话。
...
- 破坏性的文件夹移除不属于 Workspace 删除；删除 Workspace 只清理应用维护的会话数据（参见[决策记录](../../../.agents/notes/implemented/feature/2026-09-08-workspace-deletion-cascades-sessions.zh.md)）。
