...
- **Coverage report** (`pnpm run test:coverage`): reports line, branch, function, and statement coverage for `packages/*/*/src`. Coverage shows which paths tests execute, but behavior-focused assertions and integration tests establish whether the feature works as shipped.
