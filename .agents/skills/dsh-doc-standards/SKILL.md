...
- [docs/i18n/README.md](../../../docs/i18n/README.md) — the Chinese-only documentation policy and treatment of legacy translated files.
...
- Existing translated files do not require counterpart updates or consistency-record refreshes; maintain only the Chinese content needed by the task.
...
Run at least `pnpm run doc-sync`, `pnpm run lint`, and `git diff --check`; JSDoc changes may regenerate catalogs. Do not run translation pairing or create language counterparts for routine documentation changes. The PR body should give word deltas, explain any deliberately long exception, and list checks.
