...
目录轻量复核先接收最终章节任务、写作维度、材料研究用途和分支粒度结论；具体结构问题在进入 Final Check 前由对应研究分支有限修复。Final Check 对照 S3 原任务、S2 要求、用户修改、S4 差异和全书职责，先审任务调整，再审资料。程序生成任务、逐条材料用途和父总述待审项；空材料章节仍有任务项，baseline 不算已审。运行内引用与当前版本绑定；任务变化使本章材料及祖先总述结论失效，材料来源或用途变化使该关联待审。修正后的新版本仍须复核，无参数 finish_final_check 按实际记录计算漏项与阻断。越界问题必须修正或阻断，Final Check 不获得结构编辑权限。
...
正式 Evidence Map v10、分块索引、S5 输入及前端接口不变。私有 checkpoint v4 保存目录操作、粒度结论、任务操作和复核记录；缺少新事实的旧检查点必须重置 S4，恢复时重新审查 Final Check。正式产物在复核与整体验证后发布，沿用有限修复、关键写入和 Host 回滚。候选 Web 快照保留至发布后按引用裁剪。
