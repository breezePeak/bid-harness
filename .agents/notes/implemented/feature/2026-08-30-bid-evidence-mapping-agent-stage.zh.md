...
S3 完成权属于 Validator。它校验 Artifact schema、S2 标识完整覆盖、manifest 文件与解析状态、分块归属、链接路径和引用行号。只有通过校验，Host 才记录阶段完成并推进到 `outline_generation/pending`。任何失败的自动非用户阶段都通过现有通用重试操作重试。当前未接入 Web Research，因为 S3 没有可以复用的稳定 Web Tool 注册。
...
S3 在不增加阶段专用浏览器流程或状态机的前提下扩展现有控制面。Validator 会在 Agent 轮次后增加本地文件读取，重试会替换陈旧 S3 输出。公开标准等外部研究仍是后续能力。
