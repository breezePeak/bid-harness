...
Main Agent 通过 `bid_stage_inspect` 读取确认目录、招标要求与资料映射，负责解释要求、识别关键歧义与冲突并生成全书及逐节任务契约。只有用户确认安排或明确授权直接开始后，Main Agent 才调用 `bid_confirm_writing_plan`。Host 从当前询问边界后的会话事件保留用户原话，并把已确认计划写入 `chapters/writing-plan.json`；跨 Session 确认时使用工具提交的原文。计划与确认目录哈希绑定，版本由 Host 单调递增。
...
章节关系规划接收完整确认计划。Writer 接收全书指令和当前章节任务契约；Reviewer 保留既有审核并追加当前章节动态条件。文档级核验读取完整计划，对章节与整书条件逐项收口。

运行中或已完成的 S5 接受普通消息，由 Main Agent 区分问答与新任务。问答不改变阶段或 Subagent；新任务提交受影响的可写叶节后，Host 把新计划送入同一调度器。未开始章节自然采用新计划，已完成或运行中的受影响章节定向失效，其他 Writer 与 Reviewer 继续执行。Section epoch 阻止旧版本结果越过计划版本边界提交，`chapters/applied-writing-plan.json` 记录执行日志采用的版本。
...
写作计划 schema v2 和确认目录哈希成为 S5 执行输入；缺失、损坏、目录不匹配、章节集合不完整或条件 evaluator 非法都会阻止启动。S5 明确重置会删除 `chapters`，因此也删除询问标记和计划并重新询问；执行失败后的普通重试复用同一确认计划。
