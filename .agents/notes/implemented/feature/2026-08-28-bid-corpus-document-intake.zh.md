...
`BidWorkspace.import()` 先保存原始字节，再把 PDF、DOCX 和 DOC 交给 `extractDocument()`。每份文档拥有 `corpus/<stored-name>/`；manifest 版本 2 记录语料目录、正文、结构和元数据路径。TXT、Markdown、XLS 和 XLSX 保留确定性转换，但也在相同语料布局下发布 `document.md`。`messageInventory()` 投影工作区相对的正文和结构路径，使现有 `grep` 与 `read` 工具直接处理已存储语料。
...
每个受支持的投标入库文件都有一个语料正文路径，三种 Office 文档格式共享唯一生产 parser。DOC 在受支持的 Node 平台上无需安装步骤即可工作。二进制 Word 样式和复杂 PDF 表格仍会有损，扫描 PDF 需要后续 OCR 阶段，DOC/DOCX 分页仍为未知。系统会拒绝 manifest 版本 1，不会用新字段集合解释旧格式。
