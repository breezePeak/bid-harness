...
S4 执行依赖支持 fresh continuable spawn、深度限制、工具过滤和 persona 的 Subagent Provider；缺失能力会在任务开始前明确失败。并发 Child 的完成顺序不影响最终合并顺序，单个 Child 的模型输出问题只消耗该任务的修复额度，限流错误另有有限 Host 重试预算。目录深化新增一次 Main Agent 编写和复核，以及至多一批面向变化 Section 的补充研究。
