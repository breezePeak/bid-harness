...
运行态公开回合看到阶段 scoped 工具及项目级只读检查。`bid_stage_inspect` 从项目文件与 Session Log 生成有界只读快照，不取得 mutation lock：S1 返回导入计数，S2 返回分析产物摘要，S3/S4 返回目录和 Mapping 进度，S5/S6 返回最多一百个章节的写作状态、最近问题及页数估算；S4 working outline 缺失时只用 `initial-confirmed-outline.json` 恢复章节摘要和可写 ID，不伪造 Draft。最近公开事件最多六条且逐条截断。详细任务契约只在显式 `task_contract_context` 请求中返回，正文只按结构化引用读取。`bid_project_inspect` 按项目对象与真实章节 ID 分页读取已保存产物，不以当前阶段限制纯读取；其契约见[章节能力与项目只读上下文](../architecture/2026-09-24-bid-capability-contract.md)。
