...
S4 在用户确认前增加初稿和强制复核两次 Agent 回合。质量报告是 S4 内部产物，不进入 Stage Artifact 或 S5/S6 输入；S5 允许用户使用当前目录，或者提交非空修改意见重新执行 S4。修改意见记录在 `bid.user_confirmation.received` 事件中，下一轮 S4 从 Session 日志读取并作为必须落实的目录生成输入，完成后重新进入 S5。S5 继续对用户编辑或重新生成的目录使用同一组树结构和叶子粒度规则。一个技术主题确需引用更多要求时必须拆成多个可独立写作的叶子，因此后续章节写作任务更聚焦，但目录节点数量会增加。
