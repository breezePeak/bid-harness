...
Bid 面板可以选择文件，但 S1 没有把浏览器字节送入项目工作区的生产路由。通过 `session.prompt()` 发送这些字节会让通用聊天消息成为工作流权限来源，而由浏览器推进阶段则会产生无法从 Session 日志恢复的状态。

## Decision

Bid 包导出 browser-safe 上传请求与结果类型，并生成唯一的 Typert Remote `bid/uploadFiles`。Host 解析实时 Session，要求其解析后的 Preset 为 `bid`，只从 `Session.header.cwd` 获取工作区根目录，应用 Host 配置的文件限制，并以项目锁串行化接入。MVP 请求通过规范 base64 JSON 携带一个完整批次。
...
项目产物与跨 Session 进度由[Workspace 项目存储](../architecture/2026-09-03-bid-workspace-project.md)负责；本记录保留专用上传准入与浏览器权限的决定。
