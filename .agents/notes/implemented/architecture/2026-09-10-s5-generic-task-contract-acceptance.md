...
**正文可修复的动态条件失败时创建新 Writer。** 不采用；原 Writer 已持有本章研究、短引用和修复历史。定向修复续用相同身份，Reviewer 仍保持独立；外部资料缺口不调用 Writer。
...
章节报告 schema v7、完成账本 schema v2、执行计划和执行日志 schema v3 记录动态验收、外部资料缺口、投标人直接作答质量与输入身份；客户正文质量规则由[标书内部追踪身份与客户正文分离](../bug-fix/2026-09-11-bid-customer-facing-prose-boundary.md)拥有。确定性能力只有显式注册的 metric；新增 metric 需要定义可重现输入、不可用语义和边界测试，但不需要增加需求关键词分支。
