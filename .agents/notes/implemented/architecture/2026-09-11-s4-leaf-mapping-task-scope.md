...
每个完成任务把成功 `read_source` 返回的本地材料引用和已读 Web Source 身份保存为 `research_candidates`。动态子任务只把这些记录作为检索入口；Host 不据标题、关键词或父任务判断生成 `local_materials` 或 `web_materials`，子任务仍须按自身 Requirement、Scoring、Response Point 和职责读取、判断并显式提交。私有 checkpoint schema 为 v11，正式 Evidence Map schema 为 v11；Web Chunk 边界见[S4 Web Research Pool](2026-09-14-s4-web-research-pool.md)，S5 不读取 Mapping Task、代次或研究候选。
