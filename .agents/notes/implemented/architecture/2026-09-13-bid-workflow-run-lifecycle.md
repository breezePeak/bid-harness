...
每个 Long Run 在开始前持久化请求正文及输入摘要，Work Descriptor 用 `kind + workId + requestRef + requestSha256 + inputFingerprint` 绑定其中一种明确工作：完整阶段执行、文件接入、资料重映射、目录重生成、目录确认或独立章节修订。恢复按 `kind` 分派适配器并复用同一 workId 的私有工作树，不能只根据 stage 猜测工作；S4 检查点另以任务语义范围、确认目录、分析产物、语料身份和 Resume Policy 的局部指纹决定复用。
