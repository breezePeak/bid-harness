...
S2 只保存 Project、Requirements、完整 Scoring 原文和 Compliance，不包含响应点。S3 由 Agent 按评分语义生成候选响应点，Host 按评分 Artifact 哈希和单调序列分配稳定 `RP-*` ID，再结合人工框架生成初始目录。S3 校验通过后保存 `outline/initial-confirmed-outline.json`，用户可在同阶段编辑、重新生成或确认。

S4 Evidence Map schema v8 以 `section_mappings` 为唯一业务索引。每个可写 Section 恰好一条记录，包含本地材料、Web Snapshot、缺口和写作维度。本地资料保存 `source_kind + file_id + chunk`；普通 `reference` 只允许 `reference` 或 `background`，旧标书 `reference_bid` 还允许 `reuse` 与 `adapt`。Mapping Child 可以返回临时 URL，Host 只把当前 search-to-fetch 结果绑定为 `source_id + snapshot_path` 后写入最终 Evidence Map，并以来源账本和正文哈希验证本地 Snapshot。S4 提供最终目录确认。

S5 只认 `confirmed-outline.json` 的章节结构，并按 Section 直接读取 Evidence Map。Writer 候选通过确定性检查后立即持久化正文与 Metadata，随后启动独立 Reviewer。Reviewer 要求修复时最多再启动一次 Writer；第二次审查仍有明确问题则保存报告并投影为 `needs_attention`，不阻断后续章节或 S6 导出。
...
**把人工框架继续视为 Evidence。** 不采用，因为框架只能决定目录骨架，不能证明当前项目事实或正文内容；S4 最终确认后由目录完整吸收其作用。
...
Validator 证明文件角色、分块归属、Workspace 路径、目录引用、Section 覆盖、来源账本和 Snapshot 哈希，但不判断 Agent 的资料选择、目录拆分或正文改写质量。目录与正文语义质量分别由 S3 Quality Review 和 S5 Reviewer 负责。
