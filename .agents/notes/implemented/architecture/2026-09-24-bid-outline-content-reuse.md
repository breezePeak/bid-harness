...
步骤内的 `defer_content_migration` 表示迁移在同一任务中延后；任务接纳及后续计划补丁要求其后存在 `chapter.reorganize` 和正文写作或审核。该校验不改变历史请求的读取条件，已有项目的任务查找与完成记录仍可使用。任务级 `allow_pending_content` 单独表达用户明确只改目录或暂缓正文的授权。默认发布路径比较正式项目与候选的待迁移来源，拒绝新增未分配正文；既有其他任务遗留的待迁移来源不阻断无关修改。`outline.refine` 的模型生成操作也受发布检查约束。
...
**仅依赖提示词要求完整执行。** 模型仍可能把步骤级暂缓当成任务完成，提交单个目录步骤。接纳校验让模型在写入前补全计划，发布检查防止实际产物与计划不一致时产生完成凭据。
...
Main Agent 的任务说明要求已写章节的拆分或合并提交目录调整、原文迁移和复核的完整计划；用户明确只改目录时才留下待迁移正文。直接结构操作也可能请求一次模型分配业务引用，无法完成覆盖校验时不发布候选。能力测试覆盖自动分配、空分配拒绝、未迁移正文的发布拒绝及明确暂缓；源码 Loader 会话回放覆盖真实 `bid_run_task` 对不完整计划的拒绝和纯目录授权后的拆分结果。Host 校验步骤和产物，用户是否明确暂缓的语义仍由 Main Agent 判断。
