...
Research Ready 后，Child 根据关键发现判断职责、目标、方法、输入输出或验证方式不同的主题是否值得在 S5 独立论证，并检查兄弟章节重复。每个 `apply_section_outline_edit` 的 basis 引用当前 assessment 的一个 `finding_ref`；后续 assessment 必须继续包含全部已经用于目录操作的引用，缺失时 Host 拒绝更新并保留已执行的目录操作。Host 只验证引用归属，不判断 finding 与操作的语义匹配。主题归位、`outline_capacity` 的非授权语义和目录锁定遵循[主题归位驱动目录锁定](2026-09-11-s4-topic-disposition-outline-lock.md)。Writing Brief 和材料映射仍沿用现有工具、锁定与完成流程。

私有 S4 checkpoint schema v9 保存 Initial、Repair 和动态新叶任务的最终 Research Assessment、`topic_dispositions`、研究候选、稳定 finding ref 及每项目录操作的 finding basis。恢复返回该结论，Repair Prompt 接收当前 Section 子树已保存结论，全局 `reviewRefinedOutline()` 继续用现有 `blocking_issues` 定位 `MAP-REPAIR-*`；详细复核输入和遗漏深化规则由[主题归位驱动目录锁定](2026-09-11-s4-topic-disposition-outline-lock.md)约束，不增加第二套目录审核。任务与编辑作用域遵循[逐叶研究任务](../architecture/2026-09-11-s4-leaf-mapping-task-scope.md)。
...
每个 Initial 与 Repair Section 增加至少一次结构化工具调用，研究不足或目录承载结论改变时会有重复评估。Host 能证明目录和 Writing Brief 操作发生在充分结论之后、最终 Research Assessment 覆盖全部已用于目录修改的研究发现，并能把同一结论用于恢复、局部 Repair 和全局复核；研究内容、充分性和操作语义仍由模型负责。正式 Evidence Map v10、目录 Artifact 和 S5 消费语义不变；私有 checkpoint 为 v9，旧私有 checkpoint 需要重置 S4。
