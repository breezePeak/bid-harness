...
S3 Blueprint Quality Review 动态注册 `submit_outline_quality_review`，只接受 `issues=[{code,severity:"advisory",message}]`。参数错误作为工具参数错误留在当前模型轮次修正；未提交时只允许一次没有目录写权限的协议续行。唯一一次完整复核可以修改目录，但模型必须在该轮内自检结果；Host 随后规范化并确定性校验复核后的目录，为其生成 schema version 4、scope、全部 checked IDs 和 reviewed section IDs，再原子写入目录与正式质量报告。不存在兼容旧格式或候选文件的读取路径，恢复规则见[S3 有界执行与 Artifact 恢复](2026-09-21-s3-bounded-outline-generation-recovery.md)。
