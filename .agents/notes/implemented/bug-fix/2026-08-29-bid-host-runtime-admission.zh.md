...
`@deepseek-ai/dsh-bid/control-plane` 只导出浏览器安全的常量和类型，`bid.runtime` 是唯一的 Bid 投影键。Bid Host 插件注册该投影，并在 `createUserMessage`、follow-up 或 steer 执行前，根据 Session 解析出的 `bid` preset 拒绝通用提示词准入。在专用操作路由完成前，投影只暴露文件上传，并使用规范原因禁用输入框。Reducer 只接受当前阶段的转换，并忽略非法跳转。
