...
`@deepseek-ai/dsh-bid/control-plane` 只导出浏览器安全的常量和类型，`bid.runtime` 是唯一的 Bid 投影键。Bid Host 插件使用已配置的文件限制注册该 Projection，并在 `createUserMessage`、follow-up 或 steer 执行前，根据 Session 解析出的 `bid` Preset 拒绝通用 Prompt 准入。ApiProxy 从根 Context 分发 Prompt 准入事件，使同一 Host composition 中的同级插件参与统一决策。在专用操作路由完成前，Projection 只暴露文件上传，并使用规范原因禁用输入框。Reducer 只接受当前运行阶段完成；目录确认只有在要求确认并收到 `confirmed: true` 后才会进入运行状态。Projection state version 2 使旧转换规则生成的缓存失效。
