...
S3 Host 读取并解析 `analysis/scoring.json`，把完整评分数据直接注入唯一的响应点分析 Child。该 Child 使用 `scoringResponsePointCandidateSchema` structured output、空工具 allow-list，并在同轮完成语义自检。Host 校验 `schema_version`、非空 `scoring_id`、正整数 `order`、非空 `text`、评分 ID 归属、每项至少一点与从 1 开始的连续顺序。通过后，Host 使用 Run Commit Scope 将 Candidate 序列化到当前 Run 的 scratch 路径，再分配稳定 `RP-*` 并生成正式清单；非法输出不写新 Candidate，也不覆盖已有合法文件。完整有界流程由[S3 有界执行与 Artifact 恢复](2026-09-21-s3-bounded-outline-generation-recovery.md)记录。
...
S3 响应点 Child 不再读取或写入文件，Candidate 的字节表示与发布生命周期由 Host 拥有；一个 structured output 同时承担语义分析和同轮自检。极端情况下，合法工具参数若包含 128 个连续完全相同的非空流片段或超过 512 KiB，当前模型请求会明确失败；这个有界取舍防止未闭合调用无期占用 Child 与 Host 等待链。
