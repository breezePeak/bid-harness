 ...
 * with the exact runtime vector used after the Host build wrapper and assert
 * the required-config diagnostic. The Node compatibility matrix runs this
...
describe('dsh SOURCE launcher (node --import tsx/esm)', () => {
  it('keeps dsh on the Host-build source launcher', async () => {
    const rootPackage = JSON.parse(await readFile(new URL('../../../package.json', import.meta.url), 'utf8')) as {
      readonly scripts?: Record<string, string>
    }
    expect(rootPackage.scripts?.dsh).toBe('tsx scripts/run-host-dev.ts dsh')
  })
