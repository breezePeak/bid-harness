...
describe('web e2e: live-turn interactions (cancel / error / retry)', () => {
  ...
  it.skipIf(MODE === 'record')('surfaces a non-retryable AUTH failure without retrying', async () => {
    ...
    const errorDisclosure = errorStatus.getByRole('button')
    expect(await errorDisclosure.getAttribute('aria-expanded')).toBe('false')
    ...
    await errorDisclosure.click()
    expect(await errorDisclosure.getAttribute('aria-expanded')).toBe('true')
    await errorStatus.getByText('API key is invalid', { exact: true }).waitFor({ timeout: 10_000 })
