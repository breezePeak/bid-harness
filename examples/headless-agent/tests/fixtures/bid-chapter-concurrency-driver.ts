...
import { BidWorkspace, buildBidStageTask, createTestBidRunContext, executeChapterWriting, validateChapterWriting } from '@deepseek-ai/dsh-bid'
...
try {
  ...
  const artifacts = await executeChapterWriting(agent, workspace, buildBidStageTask('chapter_writing'), {
    maxRepairAttempts: 3, maxConcurrency: 3, run: createTestBidRunContext(),
  })
